...
**Hardened sky:** board level, careful 65 → 54 %, casual 29 → 12 %.

## Addendum 4 (19 Aug, night): Blaze tuning vs the sky as actually shipped

JS re-port of the engine **from the prototype's code** (`uploads/manzil-blaze-sim-2.json`), which Addendum 2's Python predates in one respect: the shipped sky has the **seeded tiebreak ON** (FNV date-seed picks among her evaluation-tied moves). 28 tonights × both leads × 20 reps, default five at prototype levels (Crown L2 to match Addendum 2).

| Blaze variant | 2-ply board % (match) | 1-ply board % | tied → sky | flips/board |
|---|---|---|---|---|
| **A — as coded** (ground full weight) | **59.0 (65.1)** | 29.7 | 20.6 % | 2.1 |
| B1 — ground counts one, rest to card owner | 58.7 (65.5) | 29.8 | 19.9 % | 2.6 |
| B2 — one to you, extras void | 56.5 (61.1) | 31.1 | 21.2 % | 2.3 |
| C — brand dies when Blaze is claimed | 36.3 (20.4) | 23.0 | 22.0 % | 1.7 |

Reading: **against the sky as shipped, the overshoot does not reproduce.** The seeded tiebreak (absent from Addendum 2's run) is worth roughly the 6 points between 64.7 and 59.0, and 59 % boards sits inside the 55–65 target band. B1 and B2 are within noise of A while complicating the rule text; C is an ablation in disguise (the brand only ever matters after a claim) and kills the card. **Call: keep Blaze as coded; no tuning needed.** The remaining gap is at the other end: 1-ply lands 29.7 % vs the 35–45 target, which is not a Blaze problem — it is where the walkers' ladder (Addendum 3) or her temperament tie-breaks would act. Caveat: different RNG and language than Addendum 2; the A-vs-B comparison is clean, the 64.7-vs-59 attribution is inferred. First-to-5 points with the cube (leader calls, Crawford at match point): careful wins ~64 % of matches (per-night spread 8–100 % across sampled mansions), casual ~8 %; mean 4.3 boards per match, wall-clock unchanged from Bo5. Reading: the ladder is a real ramp inside ~3 minutes, the hardened sky is a genuine boss for careful play and a wall for casual play, and the per-mansion spread is the argument for the 28 house rules: some nights are already bosses by dominion alone, others need their rule to bite.
