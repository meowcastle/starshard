...
// ---- L3: the Four Symbols. mansions_table.json's fy_god column; mansion 2 is blank
// there and belongs to Byakko by position. each quadrant grants a different thing at L3.
const QUADRANT = {};
[[ "byakko",  [1,2,3,4,5,6,28], "guard"  ],    // white tiger, west, metal
 [ "suzaku",  [7,8,9,10,11,12,13], "strike" ], // vermilion bird, south, fire
 [ "seiryuu", [14,15,16,17,18,19,20], "turn" ],// azure dragon, east, growth
 [ "genbu",   [21,22,23,24,25,26,27], "return" ]  // black tortoise, north, water
].forEach(([nm, ids, grant]) => ids.forEach(id => QUADRANT[id] = { nm, grant }));
// the grant a card actually has right now: only at level 3 or above, and only for mansions.
function grantOf(g, c) {
  if (!c || c.lvl < 3) return null;
  if (c.id >= 101 && c.id <= 105) return null;        // planets have no quadrant
  const q = QUADRANT[c.homeM != null ? c.homeM : c.id];
  return q ? q.grant : null;
}
function makeCards(opts) {
  ...
  POOL.forEach((p, idx) => {
    ...
    const q3 = lvl >= 3 && QUADRANT[id] ? QUADRANT[id].grant : null;
    C[id] = { id, name: p[0], l, r, ab: o.silence === id ? null : p[3], lvl, who: "you", twoFaced: p[3] === "throne" || q3 === "turn", homeM: id };
  });
...
function tryFlip(g, slots, ai, ti, dir) {
  ...
  const tie = av === tv && !stormFace && !(tGrant === "guard" && (isHome(g, t.id, ti) || [-1,1].some(d => { const k = nb(g, ti, d); return k >= 0 && isHome(g, t.id, k); })));   // byakko holds a tie on its own ground
  ...
  if (tGrant === "return" && !g.retUsed && isHome(g, t.id, ti)) return "return";   // genbu: on its own ground, once a board
  t.owner = a.owner;
...
function lodge(g, slotsIn, cardId, i, rev, side) {
  ...
  if ((c.ab === "return" && on(g, c)) || grantOf(g, c) === "return") slots[i].came = g.retUsed ? true : false;
...
function resolve(g, slotsIn, cardId, i, rev, side) {
  ...
  while (queue.length) {
    ...
    const fromGrant = grantOf(g, g.C[slots[from].id]);
    if (fromAb === "mars" || (fromGrant === "strike" && isHome(g, slots[from].id, from)) || (fromAb === "drum" && on(g, g.C[slots[from].id]))) { const far = nb(g, to, dir); if (far >= 0) queue.push({ from: to, to: far, dir }); }
  }
...
// L4: the standard bearer. one unleashed card, chosen by the player, is guaranteed into the
// five alongside tonight's mansion. exactly one - more than one and the hand stops varying,
// which is the pack-of-five condition that made the opening solvable.
function deal(pack, seed, tonight, guarantee, standard) {
  ...
  if (standard != null && standard !== tonight && bag.includes(standard)) out.push(bag.splice(bag.indexOf(standard), 1)[0]);
...
const API = { POOL, PLANETS, QUADRANT, grantOf, makeCards, mkGame, on, nb, isHome, faceOf, safeNow, tryFlip, lodge, resolve, counts, slotW, ctxOf, boardWinner, bestYouReply, skyMove, youMove, playBoard, deal, VECTORS, runVectors };
