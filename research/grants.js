...
const E = require("/tmp/now2/ref-l34.js");
const fs = require("fs");
...
console.log("level 3. her hand is seven. " + N + " boards a cell, standard error ~" + se.toFixed(1) + "pp.\n");

console.log("=== Q3: WHOSE HAND CARRIES THE GRANTS ===");
console.log("                        rung 5            rung 9 (the sky)");
console.log("config               careful  casual  gap   careful  casual  gap");
for (const [label, sides] of [["nobody (today)","none"],["the player only","you"],["both sides","both"],["the walker only","sky"]]){
  const a5=cell(5,sides,8,null), b5=cell(5,sides,0,null);
  const a9=cell(9,sides,8,null), b9=cell(9,sides,0,null);
  console.log("  "+label.padEnd(19)+a5.toFixed(1).padStart(6)+b5.toFixed(1).padStart(8)+(a5-b5).toFixed(1).padStart(6)
    +a9.toFixed(1).padStart(9)+b9.toFixed(1).padStart(8)+(a9-b9).toFixed(1).padStart(6));
}

console.log("\n=== Q2: WHAT L4 DOES, on top of both-sides grants ===");
console.log("                        rung 9 careful   rung 9 casual   gap");
for (const [label, l4] of [["L3 only","null"],["L4 = drop the condition","uncondition"]]){
  const v = l4 === "null" ? null : l4;
  const a=cell(9,"both",8,v), b=cell(9,"both",0,v);
  console.log("  "+label.padEnd(24)+a.toFixed(1).padStart(8)+b.toFixed(1).padStart(15)+(a-b).toFixed(1).padStart(8));
}
