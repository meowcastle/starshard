...
const HEAD = "  hand vs hers   surp  careful  casual    gap   unplayed   close%  blowout%";

console.log("Level 2 rules, grants and taps off. 784 boards a cell, SE 1.8pp.");
console.log("'surp' = cards dealt beyond the nine slots. 'unplayed' = cards left in the player's hand.");
console.log("'close' = decided by one slot or less. 'blowout' = decided by four or more.\n");

console.log("=== A. TODAY: the player is stuck at five while she climbs (rung 9, the sky) ===");
console.log(HEAD);
for (const her of [5,6,7,8]) row(`5 v ${her}`, 9, 5, her, 9);

console.log("\n=== B. MATCHED: the player climbs with her ===");
console.log(HEAD);
for (const n of [5,6,7,8]) row(`${n} v ${n}`, 9, n, n, 9);

console.log("\n=== C. Does a longer road do the same job? (7 v 7) ===");
console.log(HEAD);
for (const len of [9,11,13]) row(`7 v 7, road ${len}`, 9, 7, 7, len);

console.log("\n=== D. The same matched ladder at a middle rung (rung 5) ===");
console.log(HEAD);
for (const n of [5,6,7,8]) row(`${n} v ${n}`, 5, n, n, 9);
