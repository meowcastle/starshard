...
def resolve(R, board, cid, i, rev, duel_log=None):
    ...
    if c["ab"] == "venus":
        for t in (i - 1, i + 1):
            if t < 0 or t > 8 or slots[t] is None: continue
            s = slots[t]
            if R.cards[s[0]]["ab"] == "jewel": continue
            if t > i:
    ...
    if i + 1 <= 8: queue.append((i, i + 1))
    flips = 0
    while queue:
        fr, to = queue.pop(0)
        if to < 0 or to > 8 or slots[fr] is None: continue
        if not try_flip(R, slots, fr, to, duel_log): continue
        flips += 1
        if R.cards[slots[fr][0]]["ab"] == "mars":
            far = to + (to - fr)
            if 0 <= far <= 8: queue.append((to, far))
