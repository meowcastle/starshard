...
WALKER_SIGS = {}   # {200+mansion: ability} — a walker further along their own road
WALKER_LVL = 2
...
class Rules:
    ...
    def __init__(self, owned=None, off=(), all_vanilla=False):
        ...
        for idx, (name, l, r) in enumerate(POOL):
            wid = 200 + idx + 1
            wab = WALKER_SIGS.get(wid)
            wl, wr = l, r
            if wab and WALKER_LVL >= 3:
                if wl <= wr: wl = min(9, wl + 1)
                else: wr = min(9, wr + 1)
            self.cards[wid] = dict(name=name, l=wl, r=wr, who=1, ab=wab, lvl=(2 if wab else 1),
                                   two=(wab == "throne"), home=idx + 1)
