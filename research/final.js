...
function row3(label,rung,youHand,herSize,len,o,lv){
  const out=[];
  for(const off of [0,50000,90000]){
    out.push({a:cell3(rung,8,youHand,herSize,len,o,off,lv), b:cell3(rung,0,youHand,herSize,len,o,off,lv)});
  }
  const m=f=>out.reduce((s,x)=>s+f(x),0)/out.length;
  const gaps=out.map(x=>x.a.win-x.b.win);
  console.log("  "+label.padEnd(22)+m(x=>x.a.win).toFixed(1).padStart(9)+m(x=>x.b.win).toFixed(1).padStart(8)
    +m(x=>x.a.win-x.b.win).toFixed(1).padStart(7)+("±"+(Math.max(...gaps)-Math.min(...gaps)).toFixed(1)).padStart(7)
    +m(x=>x.a.close).toFixed(0).padStart(8)+m(x=>x.a.blowout).toFixed(0).padStart(9));
}
const HEAD="  build                careful  casual    gap  seeds   close%  blowout%";
const TAPS={tapMode:true,grantSides:"both",skyCanTap:true,tapOwnCardsOnly:true};
console.log("THREE seeds, 2,352 boards a cell. does the winner hold across the whole ladder?\n");
console.log("=== 5 v 5 on a road of SEVEN, level 3 on ===");
console.log(HEAD);
for(const r of [1,3,5,7,9]) row3("rung "+r, r, 5,5,7, TAPS, 3);
console.log("\n=== the same rungs on today's road of NINE, for contrast ===");
console.log(HEAD);
for(const r of [1,5,9]) row3("rung "+r, r, 5,5,9, TAPS, 3);
