...
// THE SETTLE RECORD (15 Sep 2026, FOR-CODE-THE-FLOW item 1 + Code's cards term). The count, itemised
// into the five terms the surface captions: stations (a lodged card is one), dominion (a card on its
// home station is one more), cards (what awake signatures do to the count), law (what tonight's law
// does), dawn (the held cards). Terms are read differentially so the sum is exact by construction:
// plain = stations + dominion (no laws, no signatures); law = counts(signatures off) - plain;
// cards = counts(as played) - counts(signatures off). Where a law and a signature interact, the
// interaction sits in cards, and the note says so. dawn is dawnPoints on a full road, else zero.
// reason: the first term, in the order stations, dominion, cards, law, dawn, level, after which
// the running total's sign never changes again.
function settle(g, slotsIn){
  const s = slotsIn.map(z=>z?{...z}:z);
  const full = !s.some(x=>!x);
  if(full) lastBeat({...g, slots:s});
  const side = x => x.owner==="you" ? 0 : 1;
  const stations=[0,0], dominion=[0,0];
  s.forEach((x,i)=>{ if(!x) return; stations[side(x)]++; if(isHome(g,x.id,i)) dominion[side(x)]++; });
  const plain=[stations[0]+dominion[0], stations[1]+dominion[1]];
  // signatures off: a card table with every ab nulled, same faces
  const Coff={}; for(const k in g.C) Coff[k]={...g.C[k], ab:null};
  const off = counts({...g, C:Coff}, s);
  const asPlayed = counts(g, s);
  const law=[off[0]-plain[0], off[1]-plain[1]];
  const cards=[asPlayed[0]-off[0], asPlayed[1]-off[1]];
  const notes=[];
  s.forEach((x,i)=>{ if(!x) return; const c=g.C[x.id]; if(on(g,c) && ["district","void","chamber","listener","hideaway","drum","guide"].includes(c.ab))
    notes.push({station:i, id:x.id, ab:c.ab, owner:x.owner}); });
  const dawn = full ? dawnPoints(g, s) : [0,0];
  const total=[asPlayed[0]+dawn[0], asPlayed[1]+dawn[1]];
  const level = total[0]===total[1];
  const defender = g.first==="you" ? "sky" : "you";
  const winner = level ? defender : (total[0]>total[1] ? "you" : "sky");
  // reason: walk the terms; the first one after which the sign of the running difference is final
  const terms=[["stations",stations],["dominion",dominion],["cards",cards],["law",law],["dawn",dawn]];
  let reason="level";
  const finalSign=Math.sign(total[0]-total[1]);
  if(finalSign!==0){ let run=0, last=-1; terms.forEach(([n,t],k)=>{ run+=t[0]-t[1]; if(Math.sign(run)!==finalSign) last=k; });
    reason = terms[Math.min(last+1, terms.length-1)][0]; }
  const pair = t => ({you:t[0], sky:t[1]});
  return { stations:pair(stations), dominion:pair(dominion), cards:{...pair(cards), notes}, law:pair(law),
           dawn:{ form:g.dawn||"none", ...pair(dawn) }, total:pair(total), level, levelTo: level?defender:null, winner, reason, full };
}
...
module.exports={ POOL, QUAD, makeCards, mkGame, lodge, resolve, strike, tryFlip, hiddenFor, beliefs, bestMove, finalCounts, dawnPoints, settle,
