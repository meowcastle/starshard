...
def try_flip(R, slots, ai, ti, duel_log=None, d=None):
    ...
    if STORM_STRIKE and a_storm: av += 1
    t_storm = tC0["ab"] == "storm" and tC0["lvl"] >= 2
    stormface = (a_storm or t_storm) if STORM_MODE == "both" else \
                t_storm if STORM_MODE == "defensive" else \
                a_storm if STORM_MODE == "offensive" else False
...
def resolve(R, board, cid, i, rev, duel_log=None):
    ...
    while queue and seen < 40:
        ...
        res = try_flip(R, slots, fr, to, duel_log, d)
        if not res: continue
        flips += 1
