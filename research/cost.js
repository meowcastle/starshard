...
const E = require("/tmp/now2/ref-cost.js");
const fs = require("fs");
...
function cellB(tierName, rung, youDepth, len, seedOff, bonus, budget){
  const tiers = TIERS[tierName];
  let win=0, tot=0, close=0, blow=0, placed=0;
  for (let m=1;m<=28;m++){
    const hand = herHand(m, rung, 7);
    for (let tr=0; tr<REPS; tr++){
      const { pool, levels } = buildPool(4242+seedOff+tr*7919+m*131, tiers, m, true);
      const C = E.makeCards({ levels });
      if (bonus) for (const id of pool) if (levels[id] === 1)
        C[id] = {...C[id], l: Math.min(9, C[id].l+bonus), r: Math.min(9, C[id].r+bonus)};
      const base = E.makeCards({lvl:2});
      for (const h of hand) C[200+h.id] = {...C[h.id], id:200+h.id, who:"sky",
        lvl: h.awake?3:1, ab: h.awake?base[h.id].ab:null, twoFaced:false, homeM:h.id,
        l: base[h.id].l, r: base[h.id].r};
      for (const L of ["you","sky"]){
        const h2 = dealN(pool, 7000+seedOff+tr*100+m, m, 7, true);
        const r = E.playBoard(Object.assign({C, you:h2.slice(), sky:hand.map(x=>200+x.id),
          tonight:m, leader:L, len, depth:8, youDepth, budget}, TAPS));
        tot++; if (r.winner==="you") win++;
        placed += r.slots.filter(s=>s).length;
        const d = Math.abs(r.you - r.sky);
        if (d<=1) close++; else if (d>=4) blow++;
      }
    }
  }
  return { win:100*win/tot, close:100*close/tot, blow:100*blow/tot, placed:placed/tot };
}
function pair(tier, bonus, budget){
  const A=[],B=[];
  for(const off of [0,50000]){ A.push(cellB(tier,9,8,9,off,bonus,budget)); B.push(cellB(tier,9,0,9,off,bonus,budget)); }
  const m=(a,f)=>(f(a[0])+f(a[1]))/2;
  return { careful:m(A,x=>x.win), casual:m(B,x=>x.win), gap:m(A,x=>x.win)-m(B,x=>x.win),
           close:m(A,x=>x.close), blow:m(A,x=>x.blow), placed:m(A,x=>x.placed) };
}
console.log("A LODGING COST. you get N nights of hospitality a board. a card on its own mansion is free.");
console.log("road 9, 7 cards each, her hand at level 3 with taps. two seeds.\n");
console.log("=== does a cost widen the skill gap and cut blowouts? (every card awake) ===");
console.log("  budget       careful  casual    gap   close%  blow%   cards placed");
for (const b of [null,6,5,4,3]) {
  const r = pair("flat, all awake  ", 0, b);
  console.log("  "+(b==null?"none (today)":String(b)).padEnd(13)
    +r.careful.toFixed(1).padStart(6)+r.casual.toFixed(1).padStart(8)+r.gap.toFixed(1).padStart(7)
    +r.close.toFixed(0).padStart(9)+r.blow.toFixed(0).padStart(7)+r.placed.toFixed(1).padStart(13));
}
console.log("\n=== and does a cost make the ABILITIES worth something? ===");
console.log("  budget    every card awake   asleep +1/+1   abilities are worth");
for (const b of [null,5,4,3]) {
  const aw = pair("flat, all awake  ", 0, b);
  const sl = pair("all asleep       ", 1, b);
  const d = aw.careful - sl.careful;
  console.log("  "+(b==null?"none (today)":String(b)).padEnd(10)
    +aw.careful.toFixed(1).padStart(14)+sl.careful.toFixed(1).padStart(16)
    +((d>=0?"+":"")+d.toFixed(1)+" vs +1 numbers").padStart(24));
}
