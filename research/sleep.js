...
console.log("rung 9, 7 cards from 12, road 11, her hand 7 at level 3 with taps.");
console.log("two seeds, 1,120 boards a cell.\n");
console.log("  deck                asleep gets  careful  casual    gap   close%  blow%");
for (const [tier,bonus] of [["flat, all awake  ",0],["new: 1 L3, 5 L2  ",0],["new: 1 L3, 5 L2  ",1],["new: 1 L3, 5 L2  ",2],["new: 1 L3, 5 L2  ",3],["mid: 2 L3, 5 L2  ",2],["late: 1 L4 2 L3  ",2]]){
  const A=[],B=[];
  ...
  console.log("  "+tier.padEnd(19)+(bonus?("+"+bonus+" numbers"):"nothing").padEnd(13)
    +m(A,x=>x.win).toFixed(1).padStart(7)+m(B,x=>x.win).toFixed(1).padStart(8)
