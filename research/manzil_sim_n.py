...
PLANETS = {101: ("Saturn", 9, 5, "saturn", 26), 102: ("Mars", 8, 6, "mars", 14), 103: ("Venus", 4, 7, "venus", 22),
           104: ("Mercury", 6, 5, "mercury", 8), 105: ("Jupiter", 7, 8, "jupiter", 2), 106: ("Uranus", 8, 8, None, 5), 107: ("Neptune", 3, 9, None, 17)}
SKY_HAND = (101, 102, 103, 104, 105)
N = 9
# the prototype's owned five: id -> (lvl, ability)
PROTO_OWNED = {5: (2, "blaze"), 6: (3, "storm"), 10: (2, "throne"), 17: (1, None), 18: (2, "heart")}
...
ALL_SIGS = ("storm", "heart", "blaze", "throne", "saturn", "mars", "venus", "mercury", "jupiter", "dominion")
...
# ---------------------------------------------------------------- engine
def try_flip(R, slots, ai, ti, duel_log=None):
    a = slots[ai]; t = slots[ti]
    if t is None or t[3] == a[3]: return False
    right = ti > ai
    av = a[2] if right else a[1]
    tv = t[1] if right else t[2]
    aC = R.cards[a[0]]
    storm = a[3] == 0 and aC["ab"] == "storm"
    if duel_log is not None: duel_log.append((av, tv))
    wins = av > tv or (storm and av == tv)
    if not wins: return False
    if t[4]: return False                                   # blaze brand
    tC = R.cards[t[0]]
    if tC["ab"] == "saturn": return False
    if t[3] == 0 and tC["ab"] == "heart":
        other = ti + (1 if right else -1)
        if 0 <= other <= N-1 and slots[other] is not None and slots[other][3] == 0: return False
    branded = t[4] or (a[3] == 0 and aC["ab"] == "blaze")
    slots[ti] = (t[0], t[1], t[2], a[3], branded)
    return True


def resolve(R, board, cid, i, rev, duel_log=None):
    """Place card cid at empty slot i (rev = swap faces). Returns new board tuple + flip count."""
    c = R.cards[cid]
    slots = list(board)
    l, r = (c["r"], c["l"]) if rev else (c["l"], c["r"])
    slots[i] = (cid, l, r, c["who"], False)
    if c["ab"] == "venus":
        for t in (i - 1, i + 1):
            if t < 0 or t > N-1 or slots[t] is None: continue
            s = slots[t]
            if t > i:
                if s[1] > 1: slots[t] = (s[0], s[1] - 1, s[2], s[3], s[4])
            else:
                if s[2] > 1: slots[t] = (s[0], s[1], s[2] - 1, s[3], s[4])
    queue = []
    if i - 1 >= 0: queue.append((i, i - 1))
    if i + 1 <= N-1: queue.append((i, i + 1))
    flips = 0
    while queue:
        fr, to = queue.pop(0)
        if to < 0 or to > N-1 or slots[fr] is None: continue
        if not try_flip(R, slots, fr, to, duel_log): continue
        flips += 1
        if R.cards[slots[fr][0]]["ab"] == "mars":
            far = to + (to - fr)
            if 0 <= far <= N-1: queue.append((to, far))
...
def legal_moves(R, board, hand):
    ...
    for cid in hand:
        for rev in R.faces(cid):
            for i in range(N):
                if board[i] is None: out.append((cid, i, rev))
    return out
    ...
    for cid in hand:
        for rev in R.faces(cid):
            for i in range(N):
                ...
                b2, _ = resolve(R, board, cid, i, rev)
            ...
            for rev in R.faces(cid):
                for i in range(N):
                    ...
                    if all(s is not None for s in b2):
                        ...
                        b3, _ = resolve(R, b2, sm[0], sm[1], sm[2])
...
def play_board(R, hand, tonight, you_lead, player, rng, sky_agent=None, stats=None, duel_log=None, flip_log=None):
    ...
    while any(s is None for s in board):
        if you_turn:
            ...
            board, nf = resolve(R, board, cid, i, rev, duel_log)
    ...
    return y > s, y, s   # ties go to the sky (prototype: you > sky ? "you" : "sky")
...
def play_symmetric(R, hy, hs, you_lead, agent, rng):
    ...
    board = tuple([None] * N); hand = tuple(hy); sky = skyhand; you_turn = you_lead
    while any(s is None for s in board):
        if you_turn:
            if hand:
