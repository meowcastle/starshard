...
PLANETS = {101: ("Saturn", 9, 5, "saturn", 26), 102: ("Mars", 8, 6, "mars", 14), 103: ("Venus", 4, 7, "venus", 22),
           ...
           106: ("Uranus", 8, 8, None, 5), 107: ("Neptune", 3, 9, None, 17), 108: ("Pluto", 9, 1, None, 11)}
SKY_HAND = (101, 102, 103, 104, 105)
def sky_hand_for(tonight, vary=False):
    """shipped: always the same five. vary=True: five of the eight, chosen by night."""
    if not vary: return (101, 102, 103, 104, 105)
    pool = [101, 102, 103, 104, 105, 106, 107, 108]
    h = fnv("skyhand", tonight)
    out = []
    for _ in range(5):
        k = h % len(pool); out.append(pool.pop(k)); h = (h * 16777619 + 7) & 0xFFFFFFFF
    return tuple(sorted(out))


WALKERS = [("imra", (201, 202, 208, 211, 227)), ("sef", (203, 207, 212, 219, 222)),
           ("odel", (204, 214, 213, 226, 216)), ("brann", (206, 215, 218, 210, 220)),
           ("the quiet one", (206, 215, 218, 221, 224))]
REPLY_W = 8   # 12 on a boss board
TIERULE = "you"  # who takes a tied count: "sky" | "you" | "anchor" (holder of slot 0)
STORM_STRIKE = False  # candidate: Storm attacks with +1 on its facing number
STORM_MODE = "defensive"  # "off" | "both" (shipped 20 Aug) | "defensive" (Storm cannot BE tied) | "offensive"
SAME = True      # ties flip (Triple Triad's Same rule), both sides
COMBO = True     # a card flipped by a tie then attacks its own neighbours
...
PROTO_OWNED = {5: (2, "blaze"), 6: (3, "storm"), 10: (2, "throne"), 17: (1, "crown"), 18: (2, "heart")}
ALL_L2 = {5: (2, "blaze"), 6: (2, "storm"), 10: (2, "throne"), 17: (1, "crown"), 18: (2, "heart"), 14: (2, "jewel")}
...
ALL_SIGS = ("storm", "heart", "blaze", "throne", "crown", "jewel", "hideaway", "saturn", "mars", "venus", "mercury", "jupiter", "dominion")
...
class Rules:
    ...
    def __init__(self, owned=None, off=(), all_vanilla=False):
        ...
        for idx, (name, l, r) in enumerate(POOL):
            self.cards[200 + idx + 1] = dict(name=name, l=l, r=r, who=1, ab=None, lvl=1,
                                             two=False, home=idx + 1)
...
def try_flip(R, slots, ai, ti, duel_log=None, d=None, tonight=None):
    ...
    aC = R.cards[a[0]]
    tC0 = R.cards[t[0]]
    storm = a[3] == 0 and aC["ab"] == "storm" and aC["lvl"] >= 2
...
def counts(R, board, tonight):
    ...
    for i, s in enumerate(board):
        ...
        c = R.cards[s[0]]
        if c["ab"] == "hideaway" and c["lvl"] >= 2: continue   # the tent is silent
...
EARLY_ONLY = 0     # if >0, the epsilon band applies only while <= this many cards are down
EPS = 0.0          # widen her tiebreak: consider every move within EPS of the top
def fnv(*parts):
    h = 2166136261
    for p in parts:
        for ch in str(p):
...
def sky_move(R, board, sky, hand, tonight, daykey=0):
    """_skyMove(). With EPS>0 she picks by an FNV seed among moves within EPS of the top:
    identical for everyone on a given night, different across nights, never weaker by more than EPS."""
    scored = []
        ...
        for i in range(9):
            ...
            for rev in revs:
                ...
                if len(hand) > 0 and not full_after:
                    ...
                    sc = (s - y) * 10 - (0 if reply is None else reply) * REPLY_W
                else:
                    sc = (s - y) * 10
                sc += (c["l"] + c["r"]) * 0.1 + (8 - i) * 0.01
                scored.append(((cid, i, rev), sc))
    ...
    if EPS <= 0:
        for mv, v in scored:
            if v == top: return mv
        return scored[0][0]
    filled = sum(1 for x in board if x is not None)
    if EARLY_ONLY and filled > EARLY_ONLY:
        for mv, v in scored:
            if v == top: return mv
    cands = [mv for mv, v in scored if v >= top - EPS]
    return cands[fnv(daykey, tonight, filled, len(cands)) % len(cands)]
...
# ---------------------------------------------------------------- a board, a match
def play_board(R, hand, tonight, you_lead, player, rng, sky_agent=None, stats=None, duel_log=None, flip_log=None, sky_hand=None):
    """Returns (you_won, you_count, sky_count)."""
    board = tuple([None] * 9)
    hand = tuple(hand); sky = tuple(sky_hand if sky_hand is not None else SKY_HAND)
    you_turn = you_lead
